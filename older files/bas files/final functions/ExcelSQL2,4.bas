...
Sub GenerateSQL()
    ...
    ' Prompt user for additional table details
    userResponse = MsgBox("Do you want to provide additional details for the table (e.g., Primary Key, Foreign Key, Indexes)?", vbYesNo, "Additional Table Details")
    If userResponse = vbYes Then
        sqlCreate = sqlCreate & vbCrLf & GetAdditionalTableDetails(rngHeaders)
    End If
...
End Function
Function GetAdditionalTableDetails(rngHeaders As Range, outputTableName As String) As String
    ...
    ' Ask user about Primary Key
    Set selectedCells = Application.InputBox("Select the cells for the Primary Key columns:", "Primary Key", Type:=8)
    If Not selectedCells Is Nothing Then
        ReDim columnNames(1 To selectedCells.Cells.Count)
        For i = 1 To selectedCells.Cells.Count
            columnNames(i) = selectedCells.Cells(i).value
        Next i
        result = result & "ALTER TABLE outputTableName ADD PRIMARY KEY (" & Join(columnNames, ",") & ");" & vbCrLf
    End If

    ' Ask user about Foreign Key
    userResponse = MsgBox("Do you want to add a Foreign Key?", vbYesNo, "Foreign Key")
    If userResponse = vbYes Then
        Set selectedCells = Application.InputBox("Select the cell for the Foreign Key column:", "Foreign Key", Type:=8)
        If Not selectedCells Is Nothing Then
            ...
            result = result & "ALTER TABLE outputTableName ADD FOREIGN KEY (" & columnName & ") REFERENCES " & referenceTable & "(" & referenceColumn & ");" & vbCrLf & "ALTER TABLE outputTableName ALTER COLUMN " & columnName & " SET NOT NULL;" & vbCrLf
        End If
    End If
    ...
    If userResponse = vbYes Then
        Set selectedCells = Application.InputBox("Select the cell for the Index column:", "Indexes", Type:=8)
        If Not selectedCells Is Nothing Then
            columnName = selectedCells.Cells(1, 1).value
            result = result & "CREATE INDEX idx_" & columnName & " ON outputTableName (" & columnName & ");" & vbCrLf
        End If
    End If
    ...
    Do
        ...
        Set selectedCells = Application.InputBox("Select the cell for the Constraint column:", "Constraints", Type:=8)
        If Not selectedCells Is Nothing Then
            columnName = selectedCells.Cells(1, 1).value
            constraintType = InputBox("Enter the type of constraint (e.g., UNIQUE, NOT NULL):", "Constraints")
            result = result & "ALTER TABLE outputTableName ADD CONSTRAINT UC_" & columnName & " " & constraintType & " (" & columnName & ");" & vbCrLf
        End If
    ...
    Do
        ...
        If userResponse = vbNo Then Exit Do
        Set selectedCells = Application.InputBox("Select the cell for which you want to set the Default Value:", "Default Values", Type:=8)
        If Not selectedCells Is Nothing Then
            columnName = selectedCells.Cells(1, 1).value
            defaultValue = InputBox("Enter the default value:", "Default Values")
            result = result & "ALTER TABLE outputTableName ALTER COLUMN " & columnName & " SET DEFAULT '" & defaultValue & "';" & vbCrLf
        End If
    Loop While userResponse = vbYes
