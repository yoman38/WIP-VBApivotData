...
Sub GetUserInputAndRunSubroutines()
    ...
    ' Exit if user didn't choose a file
    ...
    ' Ask if the user knows the last row
    Dim response As VbMsgBoxResult
    response = MsgBox("Do you know the last row of your table? Do you know the column letter for Brygada? Is there a relevant column before Brygada", vbYesNo + vbQuestion, "Confirmation")
    If response = vbNo Then
        ' Display range browser
        Dim TempRange As Range
        On Error Resume Next
        Set TempRange = Application.InputBox("Navigate to your table and press cancel when done. Your selection will not be saved.", Type:=8)
        On Error GoTo 0
    End If
    ...
    On Error Resume Next
    Set lastRowRange = Application.InputBox("Please click on the last row of the table:", Type:=8)
    On Error GoTo 0
    If Not lastRowRange Is Nothing Then
    ...
    On Error Resume Next
    Set squadColRange = Application.InputBox("Please click on the 'Brygada' header cell or column:", Type:=8)
    On Error GoTo 0
    If Not squadColRange Is Nothing Then
