...
' Main sub
Function GenerateSQL(rng As Range, outputFileName As String) As String
    ...
    sqlInsert = GenerateInsertStatements(rngData, filterKeyword, outputFileName, rngFilter, rngDuplicateCheck, rngEmptyCheck)
        ...
        ' After getting the createTableStatement
    createTableStatement = GenerateCreateTable(rng, outputFileName)
    
        ' Parse the createTableStatement to build a dictionary of column names and their datatypes
    Dim colDataTypes As Object
    Set colDataTypes = CreateObject("Scripting.Dictionary")
    Dim regex As Object
    Set regex = CreateObject("VBScript.RegExp")
    regex.Global = True
    regex.pattern = "\[([\w\s]+)\]([\w\s()]+)(,|NOT)"
    
    Dim matches As Object
    Set matches = regex.Execute(createTableStatement)

    Dim match As Variant
    For Each match In matches
        colDataTypes(match.SubMatches(0)) = Trim(match.SubMatches(1))
    Next match
    
        ' Before generating the alterTableStatement
    alterTableStatement = GetAdditionalTableDetails(rngHeaders, outputFileName)

    ' Modify the alterTableStatement to include the datatype
    Dim colName As String
    Dim colType As String
    regex.pattern = "ALTER TABLE \[([\w\s]+)\] ALTER COLUMN \[([\w\s]+)\] NOT NULL"

    Set matches = regex.Execute(alterTableStatement)
    For Each match In matches
        colName = match.SubMatches(1)
        If colDataTypes.Exists(colName) Then
            colType = colDataTypes(colName)
            alterTableStatement = Replace(alterTableStatement, "ALTER COLUMN [" & colName & "] NOT NULL", "ALTER COLUMN [" & colName & "] " & colType & " NOT NULL")
        End If
    Next match
...
Function GetAdditionalTableDetails(rngHeaders As Range, outputFileName As String) As String
    ...
    Dim i As Integer
        ...
        If Not selectedCells Is Nothing Then
            ...
            For i = 1 To selectedCells.Cells.Count
                ...
                result = result & "ALTER TABLE " & outputFileName & " ALTER COLUMN " & "[" & columnName & "] " & " NOT NULL;" & vbCrLf
    ...
    Do
        ...
        If Not selectedCells Is Nothing Then
            columnName = selectedCells.Cells(1, 1).value
            constraintType = InputBox("Enter the type of constraint (e.g., UNIQUE, NOT NULL, CHECK):", "Constraints")
