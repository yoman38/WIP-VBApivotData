...
Function GetAdditionalTableDetails(rngHeaders As Range, outputFileName As String) As String
    ...
    Dim columnNamesList() As String
    ReDim columnNamesList(1 To selectedCells.Cells.Count)
    For i = 1 To selectedCells.Cells.Count
        columnNamesList(i) = selectedCells.Cells(i).value
    Next i
    result = result & "ALTER TABLE " & outputFileName & " ADD PRIMARY KEY (" & Join(columnNamesList, ",") & ");" & vbCrLf
