...
Const PROMPT_NO As String = "no"
    ...
    Dim columnDataTypes As Object
     ...
     ' Exclude Filter variables
    Dim useFilterExclude As Boolean
    Dim filterExcludeKeyword As String
    Dim rngFilterExclude As Range
        ' Newly defined variables
    Dim filterExcludeCondition As Boolean
...
Sub GenerateSQL()
    ...
    If useFilter Then
        filterKeyword = GetUserInput("Enter the keyword to filter rows. (To enter multiple keywords write without spaces: keyword1,keyword2)")
        Set rngFilter = SelectRange(ws, "Select the range for filtering. (Try to give it the same number of rows as the data range)")
    End If
    
        ' Ask if the user wants to skip rows with a specific keyword
    useFilterExclude = MsgBox("Do you want to skip rows with a specific keyword?", vbYesNo) = vbYes
    If useFilterExclude Then
        filterExcludeKeyword = GetUserInput("Enter the keyword to skip rows.(To enter multiple keywords write without spaces: keyword1,keyword2)")
        Set rngFilterExclude = SelectRange(ws, "Select the range for exclusion. (Try to give it the same number of rows as the data range)")
    End If
    ...
    ' Directly open the output file for review\n'
        If Len(Dir(outputFilePath)) > 0 Then
            Call Shell("notepad.exe """ & outputFilePath & """", vbNormalFocus)
        Else
            MsgBox "Error: Output file not found.", vbExclamation
        End If
End Sub
...
End Function
Function GenerateInsertStatements(rng As Range, filterKeyword As String, outputFileName As String, Optional rngFilter As Range, Optional rngDuplicateCheck As Range, Optional rngEmptyCheck As Range, Optional filterExcludeKeyword As String = "", Optional rngFilterExclude As Range) As String

    Dim sqlInsert As String
    Dim sqlrow As String
    Dim filterValue As String
    Dim excludeFilterValue As String
    ...
    For i = 1 To rng.Rows.Count
        ...
        ' Filter Condition
        filterCondition = False
        If Not rngFilter Is Nothing And filterKeyword <> "" Then
            filterKeywords = Split(filterKeyword, ",")
            If i <= rngFilter.Rows.Count Then
                filterValue = rngFilter.Cells(i, 1).value
                For Each keyword In filterKeywords
                    If InStr(1, CStr(filterValue), Trim(keyword), vbTextCompare) > 0 Then
        ...
        filterExcludeCondition = True
        If Not rngFilterExclude Is Nothing And filterExcludeKeyword <> "" Then
            excludeKeywords = Split(filterExcludeKeyword, ",")
            If i <= rngFilterExclude.Rows.Count Then
                filterValue = rngFilterExclude.Cells(i, 1).value
                For Each keyword In excludeKeywords
                    If InStr(1, CStr(filterValue), Trim(keyword), vbTextCompare) > 0 Then
                        filterExcludeCondition = False
                        Exit For
                    End If
                Next keyword
            Else
        ...
        ' duplicate condition
        If Not rngDuplicateCheck Is Nothing Then
            If i <= rngDuplicateCheck.Rows.Count Then
                filterValue = rngDuplicateCheck.Cells(i, 1).value
                duplicateCondition = Application.WorksheetFunction.CountIf(rngDuplicateCheck.Resize(i, 1), rngDuplicateCheck.Cells(i, 1).value) = 1
            Else
        ...
        ' empty condition
        If Not rngEmptyCheck Is Nothing Then
            If i <= rngEmptyCheck.Rows.Count Then
                filterValue = rngEmptyCheck.Cells(i, 1).value
                emptyCondition = Len(Trim(rngEmptyCheck.Cells(i, 1).value)) <> 0
            Else
                emptyCondition = True
            End If
        Else
        ...
        If filterCondition And filterExcludeCondition And duplicateCondition And emptyCondition Then
            sqlrow = "INSERT INTO [" & outputFileName & "] VALUES ("
            For j = 1 To rng.Columns.Count
                ...
                If IsDate(cellValue) Then
                    ...
                    End If
                End If
                sqlrow = sqlrow & "N'" & ReplaceSpecialCharacters(Replace(cellValue, "'", "''")) & "', "
            Next j
            ' Remove trailing comma and space, add closing bracket and semicolon
            sqlrow = Left(sqlrow, Len(sqlrow) - 2) & ");" & vbCrLf
            sqlInsert = sqlInsert & sqlrow
        End If
    Next i
...
Function SelectWorksheet(wb As Workbook) As Worksheet
    ...
    Dim msg As String
    Dim wsCount As Integer
    Dim validSelection As Boolean: validSelection = False
    
    Do While Not validSelection
        msg = "Please select a worksheet by entering its corresponding number: (press 0 or click cancel to exit)" & vbCrLf & vbCrLf
        index = 1
...
If userResponse = vbYes Then
    ' Determine the type of index to create
    Dim indexType As String
    indexType = InputBox("Enter the type of index:" & vbCrLf & "1. Non-clustered" & vbCrLf & "2. Clustered" & vbCrLf & "3. Unique Non-clustered with Sort Order", "Index Type")

    ' Prompt the user to select the header cell(s) for the Index column(s)
    Set selectedCells = Application.InputBox("Select the header cell(s) for the Index column(s):", "Indexes", Type:=8)

    If Not selectedCells Is Nothing Then
        ReDim columnNames(1 To selectedCells.Cells.Count)
        For i = 1 To selectedCells.Cells.Count
            columnNames(i) = selectedCells.Cells(i, 1).value
        Next i

        Select Case indexType
            Case "1"
                ' Non-clustered Index
                result = result & "CREATE INDEX idx_" & Join(columnNames, "_") & " ON [" & outputFileName & "] (" & Join(columnNames, ",") & ");" & vbCrLf

            Case "2"
                ' Clustered Index
                result = result & "CREATE CLUSTERED INDEX idx_" & Join(columnNames, "_") & " ON [" & outputFileName & "] (" & Join(columnNames, ",") & ");" & vbCrLf

            Case "3"
                ' Unique Non-clustered Index with Sort Order
                Dim sortOrder() As String
                ReDim sortOrder(LBound(columnNames) To UBound(columnNames))
                For i = LBound(columnNames) To UBound(columnNames)
                    sortOrder(i) = InputBox("Enter the sort order for column '" & columnNames(i) & "' (ASC/DESC):", "Sort Order")
                    columnNames(i) = columnNames(i) & " " & sortOrder(i)
                Next i
                result = result & "CREATE UNIQUE INDEX idx_" & Replace(Join(columnNames, "_"), " ", "") & " ON [" & outputFileName & "] (" & Join(columnNames, ",") & ");" & vbCrLf

            Case Else
                MsgBox "Invalid index type selected.", vbExclamation, "Error"
        End Select
    End If
End If
    ...
    Do
        userResponse = MsgBox("Do you want to add a Constraint? (e.g. UNIQUE, CHECK)", vbYesNo, "Constraints")
