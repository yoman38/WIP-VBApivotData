...
For j = 1 To numRows
    If Not IsEmpty(data.Cells(j, i).value) Then ' Skip empty cells
        ...
        sqlCreate = sqlCreate & "[" & columnName & "]" & guessedType & "NOT NULL" & ", "
    Next i
...
Function GetAdditionalTableDetails(rngHeaders As Range, outputFileName As String) As String
    ...
    If response = vbYes Then
        Set selectedCells = Application.InputBox("Select the headers cells for the Primary Key columns:", "Primary Key", Type:=8)
