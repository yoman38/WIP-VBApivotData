...
Function GetAdditionalTableDetails(rngHeaders As Range) As String
    ...
    columnNameCell = InputBox("Enter the header cell reference for the Primary Key (leave blank if none):", "Primary Key")
    columnName = rngHeaders.Range(columnNameCell).value
    If columnName <> "" Then
        result = result & "ALTER TABLE outputFileName ADD PRIMARY KEY (" & columnName & ");" & vbCrLf
    End If

    ' Ask user about Foreign Key
    userResponse = MsgBox("Do you want to add a Foreign Key?", vbYesNo, "Foreign Key")
    If userResponse = vbYes Then
        columnNameCell = InputBox("Enter the header cell reference for the Foreign Key:", "Foreign Key")
    columnName = rngHeaders.Range(columnNameCell).value
        ...
        result = result & "ALTER TABLE outputFileName ADD FOREIGN KEY (" & columnName & ") REFERENCES " & referenceTable & "(" & referenceColumn & ");" & vbCrLf & "ALTER TABLE outputFileName ALTER COLUMN " & columnName & " SET NOT NULL;" & vbCrLf
    End If

    ' Ask user about Indexes
    userResponse = MsgBox("Do you want to add an Index?", vbYesNo, "Indexes")
    If userResponse = vbYes Then
        columnNameCell = InputBox("Enter the header cell reference for the Index:", "Indexes")
    columnName = rngHeaders.Range(columnNameCell).value
    ...
    If userResponse = vbYes Then
        columnNameCell = InputBox("Enter the header cell reference for the Constraint:", "Constraints")
    columnName = rngHeaders.Range(columnNameCell).value
    ...
    ' Ask user about Default Values
    userResponse = MsgBox("Do you want to set a Default Value for any column?", vbYesNo, "Default Values")
    If userResponse = vbYes Then
        columnNameCell = InputBox("Enter the header cell reference for the Default Value:", "Default Values")
    columnName = rngHeaders.Range(columnNameCell).value
        defaultValue = InputBox("Enter the default value:", "Default Values")
        result = result & "ALTER TABLE outputFileName ALTER COLUMN " & columnName & " SET DEFAULT '" & defaultValue & "';" & vbCrLf
    End If
