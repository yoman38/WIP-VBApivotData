...
Function GetAdditionalTableDetails(rngHeaders As Range) As String
    ...
    columnName = InputBox("Enter the column name for the Primary Key (leave blank if none):", "Primary Key")
    If columnName <> "" Then
        result = result & "ALTER TABLE TableName ADD PRIMARY KEY (" & columnName & ");" & vbCrLf
    End If

    ' Ask user about Foreign Key
    userResponse = MsgBox("Do you want to add a Foreign Key?", vbYesNo, "Foreign Key")
    If userResponse = vbYes Then
        columnName = InputBox("Enter the column name for the Foreign Key:", "Foreign Key")
        ...
        result = result & "ALTER TABLE TableName ADD FOREIGN KEY (" & columnName & ") REFERENCES " & referenceTable & "(" & referenceColumn & ");" & vbCrLf
    End If

    ' Ask user about Indexes
    userResponse = MsgBox("Do you want to add an Index?", vbYesNo, "Indexes")
    If userResponse = vbYes Then
        columnName = InputBox("Enter the column name for the Index:", "Indexes")
        result = result & "CREATE INDEX idx_" & columnName & " ON TableName (" & columnName & ");" & vbCrLf
    End If

    ' Ask user about Constraints
    userResponse = MsgBox("Do you want to add a Constraint?", vbYesNo, "Constraints")
    If userResponse = vbYes Then
        columnName = InputBox("Enter the column name for the Constraint:", "Constraints")
        constraintType = InputBox("Enter the type of constraint (e.g., UNIQUE, NOT NULL):", "Constraints")
        result = result & "ALTER TABLE TableName ADD CONSTRAINT UC_" & columnName & " " & constraintType & " (" & columnName & ");" & vbCrLf
    End If
    ...
    If userResponse = vbYes Then
        columnName = InputBox("Enter the column name for the Default Value:", "Default Values")
        defaultValue = InputBox("Enter the default value:", "Default Values")
        result = result & "ALTER TABLE TableName ALTER COLUMN " & columnName & " SET DEFAULT '" & defaultValue & "';" & vbCrLf
    End If
