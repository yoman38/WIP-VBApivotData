...
Sub UpdateSQLWithTxtContent()
    ...
    ' Debugging: Check if the subroutine is being triggered
    MsgBox "UpdateSQLWithTxtContent subroutine started."
    ...
    ' Connect to SQL
    If Not ConnectToSQL(serverName, dbName) Then
        MsgBox "Connection to SQL failed."
        Exit Sub
    End If
    ...
    ' Check if gSelectedExcelFile is initialized
    If IsEmpty(gSelectedExcelFile) Or gSelectedExcelFile = "" Then
        MsgBox "The file path is not initialized. Please select a file in TPtoPIVOT first.", vbExclamation
        Exit Sub
    End If
    
    ' Debugging: Display value and type of gSelectedExcelFile
    MsgBox "Value of gSelectedExcelFile: " & gSelectedExcelFile & vbCrLf & "Type: " & TypeName(gSelectedExcelFile)

    filePath = gSelectedExcelFile
    sqlText = GetQueryFromTxt(filePath)
    ...
    ' Try executing the new query and handle any errors
    On Error GoTo ErrorHandler
